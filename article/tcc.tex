...
de memória \textit{RAM} e \textit{CPU}, \textit{thoughput} e latência do \textit{event loop} da aplicação nos três cenários propostos. 
...
com o sistema operacional \textit{Windows} 10 64 \textit{bits}, possuindo 24 \textit{gigabytes} de memória \textit{RAM} e um 
processador com 4 núcleos e 4 \textit{threads}.
...
observadas em cada requisição completada. A métrica a ser capturada utilizando esta ferramenta será o \textit{thoughput},
que se refere à quantidade de requisições feitas à aplicação por unidade de tempo.

Para uma parte do monitoramento das aplicações será utilizada biblioteca para \textit{Node.js} \textit{Autocannon}, na
versão 7.12.0 \cite{AUTOCANNON}. A biblioteca oferece opções de configuração para efetuar o disparo de requisições e coletar
dados relevantes destas. É possível, por meio de um \textit{script} feito em \textit{JavaScript}, definir a quantidade de clientes,
a quantidade de requisições que estes irão fazer, e se estas ocorreram simultaneamente. A métrica a ser capturada utilizando
esta ferramenta será o \textit{thoughput}, que se refere à quantidade de requisições feitas à aplicação por unidade de tempo.
O Código~\ref{cod:autocannon} apresenta a configuração utilizada, destacando as propriedades críticas para o teste.
A primeira delas é \textit{connections}, que define o número de conexões simultâneas, ou seja, quantos clientes estarão realizando requisições concorrentes.
Em seguida, a propriedade \textit{amount}, que representa o total de requisições a serem executadas pelos clientes durante o teste.
Por fim, a propriedade \textit{connectionRate} determina o limite máximo de requisições que um único cliente pode realizar a cada segundo.
Como propriedades auxiliares, para o acesso ao servidor, mas que não afetarão as métricas, temos a passagem da \textit{URL} do servidor,
o método \textit{GET} e o tipo de conexão no cabeçalho, \textit{keep-alive}, que deixa explícito que a conexão deve manter-se aberta
para o recebimento dos dados na resposta.

\begin{lstlisting}[caption={Implementação de uma instância do \textit{autocannon} para testes}, label=cod:autocannon]
...
Com a biblioteca \textit{autocannon}, serão configurados os 3 cenários de teste correspondentes às 3 diferentes implementações.
Para avaliar o \textit{throughput} de cada caso de teste, serão usados [X] clientes, os quais enviarão solicitações à aplicação
de destino. As requisições dos clientes não serão feitas ao mesmo tempo, mas sim de forma escalonada, com cada
cliente disparando uma nova requisição a cada segundo.
Os valores de \textit{throughput} para cada cenário serão registrados após a conclusão da série de testes e posteriormente
exibidos em um terminal na forma de uma tabela. Estes dados serão então extraídos e organizados em outra tabela,
que irá agrupar os resultados de todos os cenários para comparação.
...
que ocorre a requisição e transferência de dados. Capturas de tela destes gráficos serão feitas no \textit{dashboard} da plataforma
\textit{PM2} e dispostas de acordo com seus respectivos cenários.
...
resultado de análises rigorosas e constituem uma contribuição significativa para a compreensão do tópico em questão. Por 
meio de uma análise crítica dos resultados, procura-se não apenas responder às questões de pesquisa, mas também identificar 
tendências, anomalias e insights que possam enriquecer o corpo de conhecimento existente na área de estudo.
...
Este estudo serve como base para otimizar sistemas web em \textit{Node.js}, enfatizando a importância de escolher a 
abordagem adequada de acordo com as necessidades específicas do projeto.
