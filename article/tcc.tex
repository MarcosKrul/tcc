...
A implementação inicial para a leitura do arquivo de 1.65 GB de forma assíncrona,
...
Em resumo, serão enviadas quatro requisições ao mesmo tempo ao servidor até que se complete a quantidade total de solicitações (\autoref{cod:shelltest}).
...
com \textit{clusters}, onde serão criados processos filhos, faz-se necessário uma adaptação no código utilizado anteriormente. 
...
criando um arquivo de saída para cada, da mesma forma como mostra o código anterior (\autoref{cod:monit_multi}).
...
que sejam marcados os tempos de ínicio e de fim da bateria de solicitações. Os códigos acima, executados simultaneamente,
demonstram a recuperação dos valores de ínicio de fim da bateria de
requisições, através de operações simples para busca dos valores de máximo e mínimo, fazendo com que estes sejam
gravados no mesmo arquivo de resultados (\autoref{cod:analytics} e \autoref{cod:analytics_startend}).
...
a estes tempos (\autoref{cod:analytics_startend}).
...
para encontrar o \textit{timestamp} mais próximo dos valores de início e fim, com um erro de, no máximo, um segundo (\autoref{cod:analytics_startend}).
...
\caption{Assíncrono: Uso de memória \textit{RAM} X tempo}\label{fig:async_memory_usage}
\end{figure}
...
\caption{Assíncrono: Uso de \textit{CPU} X tempo}\label{fig:async_cpu_usage}
\end{figure}
...
\begin{figure}[H]
	...
	\caption{Assíncrono: Primeiro \textit{cluster}}\label{fig:async-cluster_usage_0}
\end{figure}
...
\begin{figure}[H]
	...
	\caption{Assíncrono: Segundo \textit{cluster}}\label{fig:async-cluster_usage_1}
\end{figure}
...
\begin{figure}[H]
	...
	\caption{Assíncrono: Terceiro \textit{cluster}}\label{fig:async-cluster_usage_2}
\end{figure}
...
\begin{figure}[H]
		...
		\caption{Assíncrono: Quarto \textit{cluster}}\label{fig:async-cluster_usage_3}
\end{figure}
