...
O \textit{Node.js} fornece o módulo de \texit{cluster} para permitir a execução de várias instâncias de aplicações na
mesma máquina. Esse módulo é semelhante ao módulo \textit{child_process}, também nativo do \textit{Node.js},
que fornece o método \texit{fork} para criar subprocessos, sendo a principal diferença entre os dois módulos
...
de memória \textit{RAM} e [INSERIR OUTRAS MÉTRICAS AQUI] da aplicação nos dois cenários propostos. O servidor de 
aplicação será baseado em \textit{Node.JS}, utilizando a versão 20.3.0. 

Para monitoramento e \textit{benchmarking} das aplicações será utilizado o \textit{software} \textit{Apache JMeter},
...
observadas em cada requisição completa. %EXPLICAR VARIAVEIS DO JMETER

A ferramenta \textit{PM2} \cite{PM2} será utilizada para a captura do uso de memória \textit{RAM} e latência do 
\textit{event loop} do \textit{Node.js}, tanto no cenário síncrono como no cenário assíncrono, para fins de comparação
...
% EXPLICAR COMO AS VARIAVEIS DO JMETER SERAO CAPTURADAS (ESCRITAS EM ARQUIVO TXT, CSV...)
% FALAR QUAIS FORMAS SERAO UTILIZADAS, TABELAS, SO TEXTO, GRAFICOS
