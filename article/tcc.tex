...
\def\lstlistingautorefname{Quadro}
...
\lstdefinelanguage{JavaScript}{
  keywords={typeof, new, true, false, catch, function, return, null, try, async, catch, switch, var, if, in, while, do, else, case, break, const, for, await, with, open, as},
...
O Quadro~\ref{cod:base} apresenta a estrutura base para criação de servidores simples usando a 
...
A primeira implementação realizada, ilustrada no Quadro~\ref{cod:sync}, realiza a leitura do arquivo de 1.65 GB
...
deve ser inserido dentro do servidor previamente implementado no Quadro~\ref{cod:base}.
...
O Quadro~\ref{cod:async} apresenta a implementação inicial para a leitura do arquivo de 1.65 GB de forma assíncrona,
...
implementado no Quadro~\ref{cod:base}.
...
O Quadro~\ref{cod:async-cluster}, ainda, foi utilizado para a terceira versão do servidor, somando a técnica de leitura
...
a implementação de leitura assíncrona, igualmente como apresentada no Quadro~\ref{cod:async}.
...
ou seja, realização da leitura de arquivos de forma síncrona (Quadro~\ref{cod:sync}).
\item Cenário 2: Implementação com arquitetura escalável para transferências da ordem de \textit{gigabytes}, 
utilizando métodos da arquitetura baseada em eventos para ler arquivos de forma assíncrona (Quadro~\ref{cod:async}).
...
\textit{clusters} (Quadro~\ref{cod:async-cluster}).
\end{enumerate}
...
requisições e automatiza o processo de envio de solicitações \textit{HTTP}. O Quadro~\ref{cod:shelltest} 
...
O Quadro~\ref{cod:analytics} demonstra o \textit{script} utilizado para calcular as métricas \textit{throughput} e 
...
O Quadro~\ref{cod:monit_single} foi implementado com o objetivo de executar o servidor e armazenar, em tempo real,
...
demonstrada no Quadro~\ref{cod:monit_multi}. Neste, são recuperados os identificadores dos processos filhos 
...
gerados automaticamente a partir dos \textit{scripts} em \textit{Python}, como mostra o Quadro~\ref{cod:base_python}.
...
filhos. Para atender a este objetivo, adequou-se o \textit{script} como mostra o Quadro~\ref{cod:multi_python} para, 
...
que sejam marcados os tempos de ínicio e de fim da bateria de solicitações. O Quadro~\ref{cod:analytics_startend}, executado
ao mesmo tempo que o Quadro~\ref{cod:analytics}, demonstra a recuperação dos valores de ínicio de fim da bateria de
...
O Quadro~\ref{cod:analytics_startend} demonstra o uso das informações de início e fim da bateria de requisições para
...
solicitações irá coincidir com algum valor monitorado. Portanto, o Quadro~\ref{cod:analytics_startend} realiza uma busca
...
\hline
\textit{Throughput} (req/min) & 0.038 & 0.179 & 0.335\\
\hline
Média do tempo de resposta (min) & 26.314 & 5.581 & 2.985\\
\hline
\end{tabular}
\end{table}
...
houve um elevado de memória, com um pico de 6300 \textit{megabytes}. Ainda, observa-se que o uso de memória mantêm-se 
constante devido ao armazenamento do arquivo em memória (\autoref{fig:sync_memory_usage}).
...
que apresenta um uso bem mais elevado e próximo ao 100\text{\%} (\autoref{fig:async_cpu_usage}).
...
Para o uso de memória \textit{RAM} (\autoref{fig:async-cluster_memory_usage_0}), esta implementação
