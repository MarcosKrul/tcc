...
de memória \textit{RAM} e \textit{CPU}, \textit{thoughput}, que se refere à quantidade de requisições feitas à aplicação por unidade de tempo,
e latência do \textit{event loop} da aplicação nos três cenários propostos. 
...
foi desenvolvido um \textit{script shell}. O mesmo permite configurar a quantidade de 
requisições e automatizar o processo de envio de solicitações \textit{HTTP}. O Código~\ref{cod:shelltest} 
apresenta um exemplo em que são enviadas dez solicitações ao servidor. Para tornar o ambiente de 
teste mais realista, é possível definir a quantidade de solicitações enviadas simultaneamente, 
utilizando a opção \textit{-P4} da ferramenta \textit{xargs}, que permite a execução de comandos em lote. O envio 
das requisições é realizado por meio do comando \textit{curl}, que especifica o caminho e o método \textit{HTTP} apropriado.

\begin{lstlisting}[caption={Exemplo de \textit{script} para disparo de requisições}, label=cod:shelltest]
...
\textit{PM2} oferece a capacidade de iniciar, parar e reiniciar aplicativos de forma eficiente, facilitando a 
gestão contínua dos serviços. Além disso, sua habilidade em monitorar a saúde dos processos em execução, 
proporcionando informações detalhadas sobre a utilização de recursos como \textit{CPU} e memória, permitirá uma 
