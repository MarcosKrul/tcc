...
desenvolvimento \textit{web}, justifica-se pelo fato de que este permite uma abordagem de alta
performance e apresenta recursos para otimização e escalabilidade. Esse \textit{framework} apresenta
...
e rápida para leitura, gravação e transformação, de forma que é possível encadear dados entre estas operações 
...
sendo consumo de memória \textit{RAM} a principal. Uma das implementação será com uma arquitetura não escalável e não 
...
A base de dados foi escolhida com base em seu tamanho em \textit{gigabytes}, com o objetivo de garatir resultados
significativos para o estudo. Buscou-se uma base ampla e abrangente, a qual contivesse um número significativo de 
registros para o processamento na plataforma.
...
\begin{enumerate}[a)]
\item Cenário 1: Implementação com arquitetura não específica para transferências volumosas, ou seja, realização 
da leitura de arquivos de forma síncrona.
\item Cenário 2: Implementação com arquitetura escalável para transferências volumosas, utilizando métodos
da arquitetura baseada em eventos para ler arquivos de forma assíncrona.
\end{enumerate}
...
desenvolvidas com \textit{Node.JS}, demonstrando como o uso de técnicas como as \textit{streams} deve ser encorajado 
quando se fala em requisições de dados volumosos na \textit{web}.
