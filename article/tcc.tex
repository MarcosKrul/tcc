...
de tempo - vazão) e a média do tempo de processamento do servidor, que se refere ao tempo total que uma requisição leva
...
O módulo nativo \textit{perf\textunderscore hooks} da plataforma \textit{Node.JS}, é utilizado em todas as implementações para a obtenção dos valores de tempo de início e fim da requisição. O tempo inicial, em milissegundos, é registrado assim que uma requisição é recebida, e o tempo final é registrado uma vez que a requisição é encerrada pelo servidor, ou seja, no evento de finalização (\autoref{cod:base}, linha 11).
...
e a média do tempo de processamento do servidor nos três cenários propostos. 
...
e a média do tempo de processamento do servidor.
...
média do tempo de processamento do servidor (\textit{avg}). Para ambas, torna-se essencial definir a soma dos tempos de processamento de
...
\begin{lstlisting}[caption={Implementação do cálculo das métricas \textit{throughput} e média do tempo de processamento}, label=cod:analytics]
...
\textit{throughput}, que terá a unidade requisições por minuto. Para o cálculo da média simples de tempo de processamento do servidor, 
...
Foram extraídos valores de \textit{throughput} e tempo de processamento do servidor,
...
\hline
Média do tempo de processamento  & 22m e 49s & 5m e 48s & 4m e 29s\\
\hline
\end{tabular}
\end{table}
...
em relação às métricas de \textit{throughput} e média de tempo de processamento do servidor (\autoref{tab:metrics}). 
...
\textit{throughput} e média de tempo de processamento do servidor.
...
\begin{figure}[H]
	...
	\begin{subfigure}[H]{0.45\textwidth}
			\centering
			\subcaption{Uso de memória \textit{RAM} X tempo}\label{fig:async-cluster_usage_0a}
			\includegraphics[width=1\textwidth]{images/pt-br/results/async-cluster_memory_usage_0.png}
	\end{subfigure}
	\hfill%
	\begin{subfigure}[H]{0.45\textwidth}
			\centering
			\subcaption{Uso de \textit{CPU} X tempo}\label{fig:async-cluster_usage_0b}
...
\begin{figure}[H]
	...
	\begin{subfigure}[H]{0.45\textwidth}
			\centering
			\subcaption{Uso de memória \textit{RAM} X tempo}\label{fig:async-cluster_usage_1a}
			\includegraphics[width=1\textwidth]{images/pt-br/results/async-cluster_memory_usage_1.png}
	\end{subfigure}
	\hfill%
	\begin{subfigure}[H]{0.45\textwidth}
			\centering
			\subcaption{Uso de \textit{CPU} X tempo}\label{fig:async-cluster_usage_1b}
...
\begin{figure}[H]
	...
	\begin{subfigure}[H]{0.45\textwidth}
			\centering
			\subcaption{Uso de memória \textit{RAM} X tempo}\label{fig:async-cluster_usage_2a}
			\includegraphics[width=1\textwidth]{images/pt-br/results/async-cluster_memory_usage_2.png}
	\end{subfigure}
	\hfill%
	\begin{subfigure}[H]{0.45\textwidth}
			\centering
			\subcaption{Uso de \textit{CPU} X tempo}\label{fig:async-cluster_usage_2b}
...
\begin{figure}[H]
		...
		\begin{subfigure}[H]{0.45\textwidth}
				\centering
				\subcaption{Uso de memória \textit{RAM} X tempo}\label{fig:async-cluster_usage_3a}
				\includegraphics[width=1\textwidth]{images/pt-br/results/async-cluster_memory_usage_3.png}
		\end{subfigure}
		\hfill%
		\begin{subfigure}[H]{0.45\textwidth}
				\centering
				\subcaption{Uso de \textit{CPU} X tempo}\label{fig:async-cluster_usage_3b}
...
\textit{throughput} e média do tempo de processamento do servidor, levando, aproximadamente,
